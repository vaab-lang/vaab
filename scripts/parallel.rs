...
#[cfg(test)]
static SAW_PARALLEL: AtomicBool = AtomicBool::new(false);
...
fn run_task(task: usize, queue: &Worker<usize>, shared: &Arc<Parallel>) {
    ...
    #[cfg(test)]
    {
        ...
        if previous > 0 {
            SAW_PARALLEL.store(true, Ordering::SeqCst);
        }
    }
    let (step, pending, wakeups) = {
        ...
        let machines = match lock(&shared.machines) {
...
#[cfg(test)]
pub(crate) fn saw_parallel() -> bool {
    SAW_PARALLEL.load(Ordering::SeqCst)
}
...
mod tests {
    ...
    #[test]
    fn two_tasks_interleave_on_different_worker_threads() {
        SAW_PARALLEL.store(false, Ordering::SeqCst);
        let source = "let lane = Channel.new(of: Int, size: 0)\n\
                      together {\n\
                      \x20   start { repeat 20000 times { let changing n = 0; n = n + 1 }; send 1 to lane }\n\
                      \x20   start { repeat 20000 times { let changing n = 0; n = n + 1 }; send 2 to lane }\n\
                      }\n\
                      receive from lane\n\
                      receive from lane\n";
        ...
        assert!(
            saw_parallel(),
            "expected two tasks to run on worker threads at the same moment"
        );
    }
}
